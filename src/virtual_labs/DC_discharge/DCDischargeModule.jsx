...
function dischargeProfileAt(frac, pressure) {
  const pN = Math.max(0.002, Math.min(2.5, pressure));
...
function TubeCanvas({ width, height, gasKey, pressure, voltage, distance, cathodeKey, bField, showLabels }) {
  ...
  useEffect(() => {
    ...
    if (bField > 0) {
      ...
      ctx.font = '9px "SF Mono","Fira Code","Menlo",monospace';  // std label size
    ...
    // Anode + plunger (extends through tube wall)
    ...
    // Plunger rod — extends from anode THROUGH tube wall to the right
    const plL = anodeX + elW;
    const plR = width - 20; // extends well past tube wall
    if (plR > plL + 5) {
      // Rod
      ctx.fillStyle = '#3a3e48'; ctx.fillRect(plL, TCY - 2, plR - plL, 4);
      // Highlight on rod top edge
      ctx.fillStyle = 'rgba(180,190,210,0.15)'; ctx.fillRect(plL, TCY - 2, plR - plL, 1);
      // End cap / actuator handle
      ctx.fillStyle = '#2a2e38'; ctx.fillRect(plR - 4, TCY - 7, 8, 14);
      ctx.strokeStyle = 'rgba(180,190,210,0.25)'; ctx.lineWidth = 1;
      ctx.strokeRect(plR - 4, TCY - 7, 8, 14);
      // Seal/bushing where rod exits tube wall
      ctx.fillStyle = '#4a4e58'; ctx.fillRect(TR - 3, TCY - 5, 6, 10);
      ctx.strokeStyle = 'rgba(180,190,210,0.3)'; ctx.strokeRect(TR - 3, TCY - 5, 6, 10);
    }
    ...
    ctx.fillText(`${gas.formula} @ ${pressure >= 1 ? pressure.toFixed(1) : pressure < 0.01 ? pressure.toFixed(3) : pressure.toFixed(2)} Torr`, TR - 16, TT + 16);
    ...
    ctx.fillStyle = 'rgba(200,200,200,0.35)'; ctx.fillText('Gas Inlet', TL - 28, TCY - 39);

    // Pressure Gauge — right side, above plunger
    const gaugeR = 22; // bigger radius
    const gx = TR + 50, gy = TCY - 34;
    ctx.strokeStyle = 'rgba(150,160,180,0.25)'; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(TR, TCY - 10); ctx.lineTo(TR + 28, TCY - 10); ctx.lineTo(TR + 28, gy); ctx.lineTo(gx - gaugeR, gy); ctx.stroke();
    // Gauge circle
    ctx.beginPath(); ctx.arc(gx, gy, gaugeR, 0, Math.PI * 2);
    ctx.strokeStyle = 'rgba(180,190,210,0.4)'; ctx.lineWidth = 2; ctx.stroke();
    ctx.fillStyle = 'rgba(20,22,30,0.85)'; ctx.fill();
    // Gauge inner ring
    ctx.beginPath(); ctx.arc(gx, gy, gaugeR - 2, 0, Math.PI * 2);
    ctx.strokeStyle = 'rgba(180,190,210,0.12)'; ctx.lineWidth = 0.5; ctx.stroke();
    // "P =" label
    ctx.font = '9px "SF Mono","Fira Code","Menlo",monospace'; ctx.textAlign = 'center';
    ctx.fillStyle = 'rgba(200,200,200,0.5)'; ctx.fillText('P =', gx, gy - 8);
    // Pressure value
    const pDisplay = pressure >= 1 ? pressure.toFixed(1) : pressure < 0.01 ? pressure.toFixed(3) : pressure.toFixed(2);
    ctx.font = 'bold 10px "SF Mono","Fira Code","Menlo",monospace';
    ctx.fillStyle = '#E8650A'; ctx.fillText(pDisplay, gx, gy + 4);
    // Unit
    ctx.font = '8px "SF Mono","Fira Code","Menlo",monospace';
    ctx.fillStyle = 'rgba(200,200,200,0.5)'; ctx.fillText('Torr', gx, gy + 13);

    // Pump — right side, below plunger, styled like gauge
    const pumpR = 22;
    const px = TR + 50, py = TCY + 34;
    ctx.strokeStyle = 'rgba(150,160,180,0.25)'; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(TR, TCY + 10); ctx.lineTo(TR + 28, TCY + 10); ctx.lineTo(TR + 28, py); ctx.lineTo(px - pumpR, py); ctx.stroke();
    // Pump circle
    ctx.beginPath(); ctx.arc(px, py, pumpR, 0, Math.PI * 2);
    ctx.strokeStyle = 'rgba(180,190,210,0.4)'; ctx.lineWidth = 2; ctx.stroke();
    ctx.fillStyle = 'rgba(20,22,30,0.85)'; ctx.fill();
    // Pump inner ring
    ctx.beginPath(); ctx.arc(px, py, pumpR - 2, 0, Math.PI * 2);
    ctx.strokeStyle = 'rgba(180,190,210,0.12)'; ctx.lineWidth = 0.5; ctx.stroke();
    // Pump label
    ctx.font = '9px "SF Mono","Fira Code","Menlo",monospace'; ctx.textAlign = 'center';
    ctx.fillStyle = 'rgba(200,200,200,0.6)'; ctx.fillText('Pump', px, py + 3);
      ...
      <div ref={cRef} style={{ display: 'flex', flex: 1 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          <div style={{ flex: 1, minHeight: 300 }}>
            ...
            <div style={{ padding: '6px 16px', fontSize: 12, color: '#888', fontFamily: sysFont }}>
              <span style={{ color: '#c44', fontWeight: 600 }}>Hint:</span> <span style={{ fontStyle: 'italic' }}>The dashed line shows the theoretical Paschen curve and updates automatically when you change the gas or cathode material.</span>
            </div>
          </div>
        </div>
        ...
        <div style={S.panel}>
          ...
          <div style={S.section}>
            ...
            <ParamSlider label="Pressure" displayVal={pressure >= 1 ? pressure.toFixed(1) : pressure < 0.01 ? pressure.toFixed(3) : pressure.toFixed(2)} value={pressure} min={0.002} max={2.5} step={0.002}
              onChange={v => setPressure(v)} unit="Torr" />
            <ParamSlider label="Electrode Gap (d)" displayVal={distance.toFixed(1)} value={distance} min={1} max={50} step={0.5}
